...
describe('loadAgentsMd user-level discovery', () => {
  it('loads user-level branded and generic files before project-level', async () => {
    await mkdir(join(homeDir, '.spectre'), { recursive: true });
    await writeFile(join(homeDir, '.spectre', 'AGENTS.md'), 'user branded', 'utf-8');
  ...
  it('does not load the same file twice when the work dir is the home dir', async () => {
    ...
    await mkdir(join(homeDir, '.spectre'), { recursive: true });
    await writeFile(join(homeDir, '.spectre', 'AGENTS.md'), 'home branded', 'utf-8');
...
describe('loadAgentsMd brand home (KIMI_CODE_HOME)', () => {
  ...
  it('ignores the real-home .spectre/AGENTS.md when the brand home is elsewhere', async () => {
    await writeFile(join(brandHome, 'AGENTS.md'), 'brand wins', 'utf-8');
    await mkdir(join(homeDir, '.spectre'), { recursive: true });
    await writeFile(join(homeDir, '.spectre', 'AGENTS.md'), 'stale real-home brand', 'utf-8');
  ...
  it('falls back to the real-home .spectre/AGENTS.md when no brand home is given', async () => {
    await mkdir(join(homeDir, '.spectre'), { recursive: true });
    await writeFile(join(homeDir, '.spectre', 'AGENTS.md'), 'fallback branded', 'utf-8');
