...
// Relative to brandHomeDir, which already IS the brand data dir (~/.spectre or
// $KIMI_CODE_HOME) — no '.spectre' segment here, or it would nest twice.
const USER_BRAND_DIRS = ['skills'] as const;
const USER_GENERIC_DIRS = ['.agents/skills'] as const;
const PROJECT_BRAND_DIRS = ['.spectre/skills'] as const;
...
export interface SkillPathContext {
  ...
  /**
   * Brand data dir — `KIMI_CODE_HOME`, or `<userHomeDir>/.spectre` by default.
   * User brand skills live directly under here as `skills/`, so this path
   * carries no `.spectre` segment of its own (that would double the prefix).
   */
...
export async function resolveSkillRoots(
  ...
  const brandHomeDir = options.paths.brandHomeDir ?? path.join(userHomeDir, '.spectre');
