...
const executableName = process.platform === 'win32' ? 'spectre.exe' : 'spectre';
...
describe('native release artifacts', () => {
  afterEach(() => {
    ...
    rmSync(resolve(artifactsDir, `spectre-${target}.zip`), { force: true });
    rmSync(resolve(artifactsDir, `spectre-${target}.zip.sha256`), { force: true });
  });
  ...
  it('packages the native binary as a zip archive and checksums the archive', async () => {
    ...
    const archivePath = resolve(artifactsDir, `spectre-${target}.zip`);
    ...
    expect(readFileSync(checksumPath, 'utf-8')).toBe(
      `${sha256(readFileSync(archivePath))}  spectre-${target}.zip\n`,
    );
  });

  it('produces a manifest from zip archive checksums', async () => {
    const releaseDir = await mkdtemp(join(tmpdir(), 'spectre-manifest-zip-'));
    const archiveBytes = Buffer.from('fake zip bytes');
    const checksum = sha256(archiveBytes);
    await writeFile(join(releaseDir, 'spectre-darwin-arm64.zip'), archiveBytes);
    await writeFile(
      join(releaseDir, 'spectre-darwin-arm64.zip.sha256'),
      `${checksum}  spectre-darwin-arm64.zip\n`,
    );

    await execFileAsync(process.execPath, [manifestScript, releaseDir, '@abelcondev/spectre@0.5.0']);
    ...
    expect(manifest).toEqual({
      version: '0.5.0',
      tag: '@abelcondev/spectre@0.5.0',
      platforms: {
        'darwin-arm64': {
          filename: 'spectre-darwin-arm64.zip',
          checksum,
        },
      },
    });
