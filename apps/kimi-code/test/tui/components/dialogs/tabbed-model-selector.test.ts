...
const TAB = '\t';
const RIGHT = `${ESC}[C`;
// chalk.bgHex(colors.primary) → background truecolor for #E94444.
const PRIMARY_BG = '48;2;233;68;68';
...
describe('TabbedModelSelectorComponent', () => {
  ...
  it('renders an "All" + per-provider tab strip', () => {
    ...
    expect(out).toContain('Spectre');
    expect(out).toContain('openai');
  });

  it('highlights the active tab with a filled background (AskUserQuestion style)', () => {
    // currentValue k2 → the active tab is "Spectre"; its cell carries the
  ...
  it('cycles provider tabs with Tab', () => {
    ...
    // tabs = [All, Spectre, openai]; active starts on All.
