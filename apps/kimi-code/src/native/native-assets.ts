...
import { SPECTRE_BUILD_INFO } from '#/cli/build-info';
...
function currentTarget(): string {
  return SPECTRE_BUILD_INFO.buildTarget ?? `${process.platform}-${process.arch}`;
}
...
export function getNativeCacheBase(options: NativeAssetOptions = {}): string {
  ...
  const cacheDirEnv =
    optionalEnvValue(env, 'SPECTRE_CACHE_DIR') ?? optionalEnvValue(env, 'KIMI_CODE_CACHE_DIR');
  if (cacheDirEnv !== null) return cacheDirEnv;

  if (platform === 'darwin') return join(home, 'Library', 'Caches', 'spectre');
  if (platform === 'win32') {
    const localAppData = optionalEnvValue(env, 'LOCALAPPDATA');
    return localAppData !== null
      ? pathWin32.join(localAppData, 'spectre')
      : pathWin32.join(home, 'AppData', 'Local', 'spectre', 'Cache');
  }

  return join(optionalEnvValue(env, 'XDG_CACHE_HOME') ?? join(home, '.cache'), 'spectre');
}
...
export function getNativeAssetCacheRoot(
  ...
  const version = sanitizeSegment(options.version ?? SPECTRE_BUILD_INFO.version ?? 'dev');
...
function ensureEntryFile(cacheRoot: string): void {
  const entryPath = join(cacheRoot, 'node_modules', '.spectre-native-entry.cjs');
  ensureFile(
    entryPath,
...
export function cleanupStaleNativeCacheForCurrent(
  ...
  const version = SPECTRE_BUILD_INFO.version ?? 'dev';
