---
name: gen-changesets
description: Use when generating changesets in the Spectre repository, including package bump selection, internal package and CLI bundle handling, bump levels, major confirmation, and English changelog wording.
---

# Generate Changesets

Spectre uses changesets to manage versions and changelogs. Spectre is an independent fork of Kimi Code; we do not publish to or send PRs upstream. The current user-facing published package is:

- `@abelcondev/spectre`: the CLI

All `@moonshot-ai/*` workspace packages are treated as internal packages, including `@moonshot-ai/kimi-code-sdk`, `agent-core`, `kosong`, `kaos`, `kimi-code-oauth`, `kimi-telemetry`, and `migration-legacy`.
...
3. **Map ignored internal changes to the affected released package.** If an ignored internal package changes CLI output or behavior, list `@abelcondev/spectre` and describe the actual user-visible or release-artifact change in the changelog text.
4. **Internal package source changes that enter the CLI bundle must manually list the CLI.** `@abelcondev/spectre` inline-bundles `@moonshot-ai/*` source, but those internal packages are devDependencies from the CLI's perspective, so changesets will not automatically propagate bumps. If a change enters the CLI output, list `@abelcondev/spectre`.
   - **Web app (`@moonshot-ai/kimi-web`) changes always enter the CLI bundle.** `@moonshot-ai/kimi-web` is ignored by changesets (see `.changeset/config.json`) and cannot be mixed with `@abelcondev/spectre` in one changeset frontmatter. Describe the web change in the changelog text, but list `@abelcondev/spectre` so the CLI release carries the bundled `dist-web` output.
...
3. If an ignored internal package change enters the CLI bundle, put `@abelcondev/spectre` in frontmatter instead of mixing the ignored package into the same changeset.
...
`@moonshot-ai/kimi-web` is ignored by changesets and must **never** appear in a changeset frontmatter. Because the web app is bundled into the CLI release artifact, any web change that ships must list `@abelcondev/spectre` instead and describe the actual web-facing change in the text.
...
- Internal package source enters the CLI bundle, but `@abelcondev/spectre` is missing.
